...
<main class="main">
    ...
    <div class="filter-bar">
        ...
        <div class="filter-group">
            ...
            <select id="currencySelect" style="min-width: 80px;">
                <option value="ARS" selected>ARS</option>
                <option value="USD">USD</option>
            </select>
        </div>
        <div class="filter-group">
            <label for="exchangeRate">Cotización</label>
            <input type="number" id="exchangeRate" value="1550" min="1" style="width: 80px; padding: 8px; border: 1px solid var(--border); border-radius: var(--radius-sm); font-family: 'Inter', sans-serif; font-size: 13px; outline: none; background: var(--bg-body); color: var(--text-primary);">
        </div>
        <div class="filter-separator"></div>
    ...
    <div class="card" style="margin-bottom: 32px;">
        <div class="card-title" id="heatmapTitle">🗺️ Mapa de Calor Mensual — Ventas por Género</div>
...
document.addEventListener('DOMContentLoaded', () => {{
    ...
    document.getElementById('currencySelect').addEventListener('change', applyFilters);
...
function resetFilters() {{
    ...
    document.getElementById('currencySelect').value = 'ARS';
    document.getElementById('exchangeRate').value = '1550';
    applyFilters();
}}
...
function applyFilters() {{
    ...
    const rate = parseFloat(document.getElementById('exchangeRate').value) || 1.0;
    currencyFactor = selectedCurrency === 'ARS' ? rate : 1.0;
    currencySymbol = selectedCurrency === 'ARS' ? '$' : 'US$';
...
function calcKPIs(data) {{
    const ingresos = data.reduce((s, r) => s + r.total * currencyFactor, 0);
...
function renderKPIs(filtered, prevFiltered) {{
    ...
    const genMap = {{}};
    filtered.forEach(r => {{ genMap[r.genero] = (genMap[r.genero] || 0) + r.total * currencyFactor; }});
    ...
    document.getElementById('kpiGrid').innerHTML =
        kpiCard('Ingresos Totales', currencySymbol + ' ' + formatNum(curr.ingresos), d1, '💰', 'purple', '', 1) +
        kpiCard('Unidades Vendidas', formatInt(curr.unidades), d2, '📦', 'green', '', 2) +
        kpiCard('Ticket Promedio', currencySymbol + ' ' + curr.ticket.toFixed(2), d3, '🧾', 'blue', '', 3) +
...
function renderEvolucion(filtered) {{
    ...
    filtered.forEach(r => {{ months.add(r.año_mes); genres.add(r.genero); const k = r.año_mes + '|' + r.genero; monthGenre[k] = (monthGenre[k] || 0) + r.total * currencyFactor; }});
    ...
    charts.chartEvolucion = new Chart(document.getElementById('chartEvolucion'), {{
        ...
        options: {{ responsive: true, maintainAspectRatio: false, interaction: {{ mode: 'index', intersect: false }},
            plugins: {{ legend: {{ position: 'bottom', labels: {{ font: {{ family: 'Inter', size: 11 }}, padding: 16, usePointStyle: true, pointStyle: 'circle' }} }},
                tooltip: {{ backgroundColor: '#1B1F3B', titleFont: {{ family: 'Inter', weight: '600' }}, bodyFont: {{ family: 'Inter' }}, padding: 12, cornerRadius: 8, callbacks: {{ label: ctx => ctx.dataset.label + ': ' + currencySymbol + ' ' + formatNum(ctx.parsed.y) }} }} }},
            scales: {{ x: {{ grid: {{ color: 'rgba(0,0,0,0.04)' }}, ticks: {{ font: {{ family: 'Inter', size: 11 }}, maxRotation: 45 }} }},
                y: {{ grid: {{ color: 'rgba(0,0,0,0.04)' }}, ticks: {{ font: {{ family: 'Inter', size: 11 }}, callback: v => currencySymbol + ' ' + (v >= 1000 ? (v/1000).toFixed(0) + 'K' : v) }} }} }} }}
    }});
}}

function renderDonut(filtered) {{
    const payMap = {{}};
    filtered.forEach(r => {{ payMap[r.metodo_pago] = (payMap[r.metodo_pago] || 0) + r.total * currencyFactor; }});
    ...
    document.getElementById('comisionValue').textContent = currencySymbol + ' ' + formatNum(totalComision);
    document.getElementById('insightComisiones').innerHTML = '<strong>💡 Análisis de pasarelas</strong>Se estiman pérdidas de <strong>' + currencySymbol + ' ' + formatNum(totalComision) + '</strong> por aranceles bancarios. Incentivar Transferencias y Efectivo para reducir el impacto financiero.';
        ...
        options: {{ responsive: true, maintainAspectRatio: false, cutout: '65%',
            plugins: {{ legend: {{ position: 'bottom', labels: {{ font: {{ family: 'Inter', size: 11 }}, padding: 12, usePointStyle: true, pointStyle: 'circle' }} }},
                tooltip: {{ backgroundColor: '#1B1F3B', titleFont: {{ family: 'Inter', weight: '600' }}, bodyFont: {{ family: 'Inter' }}, padding: 12, cornerRadius: 8,
                    callbacks: {{ label: ctx => {{ const t = ctx.dataset.data.reduce((a, b) => a + b, 0); return ctx.label + ': ' + currencySymbol + ' ' + formatNum(ctx.parsed) + ' (' + ((ctx.parsed / t) * 100).toFixed(1) + '%)'; }} }} }} }} }}
    }});
}}
...
function renderMoM() {{
    const monthGenre = {{}}, months = new Set(), genres = new Set();
    RAW_DATA.forEach(r => {{ months.add(r.año_mes); genres.add(r.genero); const k = r.año_mes + '|' + r.genero; monthGenre[k] = (monthGenre[k] || 0) + r.total * currencyFactor; }});
    const sM = [...months].sort(), sG = [...genres].sort();
...
function renderHeatmap() {{
    ...
    // Update card title to reflect selected currency
    document.getElementById('heatmapTitle').textContent = '🗺️ Mapa de Calor Mensual — Ventas en ' + selectedCurrency + ' por Género';
    ...
    charts.chartHeatmap = new Chart(document.getElementById('chartHeatmap'), {{
        ...
        options: {{ responsive: true, maintainAspectRatio: false,
            plugins: {{ legend: {{ display: false }}, tooltip: {{ backgroundColor: '#1B1F3B', titleFont: {{ family: 'Inter', weight: '600' }}, bodyFont: {{ family: 'Inter' }}, padding: 12, cornerRadius: 8,
                callbacks: {{ title: ctx => {{ const d = data[ctx[0].dataIndex]; return sG[d.y] + ' — ' + sM[d.x]; }}, label: ctx => 'Ventas: ' + currencySymbol + ' ' + formatNum(data[ctx.dataIndex].v) }} }} }},
...
function renderPareto(filtered) {{
    const bookMap = {{}};
    filtered.forEach(r => {{ bookMap[r.titulo] = (bookMap[r.titulo] || 0) + r.total * currencyFactor; }});
    ...
    charts.chartPareto = new Chart(document.getElementById('chartPareto'), {{
        type: 'bar', data: {{ labels: top30.map((e, i) => '#' + (i + 1)),
            datasets: [{{ label: 'Ventas (' + selectedCurrency + ')', data: top30.map(e => e[1]), backgroundColor: '#6C5CE7AA', borderColor: '#6C5CE7', borderWidth: 1, borderRadius: 4, yAxisID: 'y', order: 2 }},
        ...
        options: {{ responsive: true, maintainAspectRatio: false,
            plugins: {{ legend: {{ position: 'bottom', labels: {{ font: {{ family: 'Inter', size: 11 }}, padding: 14, usePointStyle: true }} }},
                tooltip: {{ backgroundColor: '#1B1F3B', titleFont: {{ family: 'Inter', weight: '600' }}, bodyFont: {{ family: 'Inter' }}, padding: 12, cornerRadius: 8,
                    callbacks: {{ title: ctx => top30[ctx[0].dataIndex]?.[0] || '', label: ctx => ctx.datasetIndex === 0 ? 'Ventas: ' + currencySymbol + ' ' + formatNum(ctx.parsed.y) : 'Acumulado: ' + ctx.parsed.y.toFixed(1) + '%' }} }} }},
...
function renderDias(filtered) {{
    ...
    const dayMap = {{}};
    filtered.forEach(r => {{ dayMap[r.dia_semana] = (dayMap[r.dia_semana] || 0) + r.total * currencyFactor; }});
    ...
    charts.chartDias = new Chart(document.getElementById('chartDias'), {{
        ...
        options: {{ responsive: true, maintainAspectRatio: false,
            plugins: {{ legend: {{ display: false }}, tooltip: {{ backgroundColor: '#1B1F3B', titleFont: {{ family: 'Inter', weight: '600' }}, bodyFont: {{ family: 'Inter' }}, padding: 12, cornerRadius: 8,
                callbacks: {{ label: ctx => 'Ventas: ' + currencySymbol + ' ' + formatNum(ctx.parsed.y) }} }} }},
